...
module.exports = NodeHelper.create({
	...
	cleanUpVideoStreamDirectory: async function () {
		...
		let files = await util.promisify(fs.readdir)(
			this.videoOutputDirectory,
			{ withFileTypes: true }
		);
		files = files.filter(
			(obj) => !obj.name.endsWith('.mp3') && !obj.name.endsWith('.wav')
		);

		const unlinkPromises = files.map((obj) =>
			util.promisify(fs.unlink)(
				`${this.videoOutputDirectory}/${obj.name}`
			)
		);
	...
	monitorRingActivity: async function () {
		...
		this.ringApi.onRefreshTokenUpdated.subscribe(
			async ({ newRefreshToken, oldRefreshToken }) => {
				...
				const currentConfig = await util.promisify(fs.readFile)(
					this.envFile
				);
		...
		for (const location of locations) {
			...
			for (const device of devices) {
				this.toLog(
					`- ${device.zid}: ${device.name} (${device.deviceType})`
				);
			}
		}
